...
/// The anchor point is typically the first input received, but it can be reset to any desired point like Anchor VWAP.
///
/// # Formula
///
...
impl VolumeWeightedAveragePrice {
    ...
    ///
    /// # Returns
    /// A new instance of `VolumeWeightedAveragePrice`.
    pub fn new() -> Self {
        Self {
...
impl fmt::Display for VolumeWeightedAveragePrice {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if self.cumulative_volume == 0.0 {
            write!(f, "VolumeWeightedAveragePrice(0)")
        } else {
            write!(
                f,
